...
void CodeGenModule::Release() {
  ...
  if (getLangOpts().OpenMPIsDevice)
    getModule().addModuleFlag(llvm::Module::Max, "openmp-device",
                              LangOpts.OpenMP);
  if (getLangOpts().OpenMP && !getLangOpts().OpenMPSimd && getLangOpts().OpenMPDynamicVariantWarmup)
    getModule().addModuleFlag(llvm::Module::Max, "dynamic-variant-warmup",
                                1);
...
void CodeGenModule::EmitGlobalFunctionDefinition(GlobalDecl GD,
  ...
  if (LangOpts.OpenMPDynamicVariant && D->hasAttr<OMPDeclareVariantAttr>()) {
    ...
    for (OMPDeclareVariantAttr *A :
      ...
      if (DeclRefExpr *DRE = dyn_cast<DeclRefExpr>(VariantRef))
        if (FunctionDecl *FD = dyn_cast<FunctionDecl>(DRE->getDecl())) {
          ...
          VariantFunctions.push_back(dyn_cast<llvm::Function>(GetAddrOfGlobal(FD)));
    ...
    auto VariantNamesStruct = builder.beginArray();

    for (int i = 0; i < (int)VariantFunctions.size(); i++) {
      llvm::Constant *VariantNameStr = llvm::ConstantDataArray::getString(
          Fn->getParent()->getContext(), VariantFunctions[i]->getName());
      llvm::GlobalVariable *VariantNameGV = new llvm::GlobalVariable(
          *Fn->getParent(), VariantNameStr->getType(), true,
          llvm::GlobalValue::PrivateLinkage, VariantNameStr);
      VariantNameGV->setAlignment(std::nullopt);
      VariantNamesStruct.add(VariantNameGV);
    }

    auto VariantNamesStructGlobal = VariantNamesStruct.finishAndCreateGlobal(
        Fn->getName() + "_variants_names", getPointerAlign(),
        /*constant*/ false, llvm::GlobalValue::PrivateLinkage);

    VariantNamesStructGlobal->setAlignment(std::nullopt);
  }
