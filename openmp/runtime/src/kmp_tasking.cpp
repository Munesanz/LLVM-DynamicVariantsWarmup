...
#define GPU 9
#define CPU 8
#define PARALLEL_VARIANT 3

#if LIBOMP_TASKGRAPH
#include <sys/time.h>

kmp_bootstrap_lock_t l_tasks;
//kmp_bootstrap_lock_t l_tasks_team;
kmp_bootstrap_lock_t l_target_tasks;
int TASKS = 0;
//int TASKS_TEAM = 0;
int TARGET_TASKS = 0;
...
// deallocate the taskdata and shared variable blocks associated with this task
  ...
  if (!taskdata->is_taskgraph) {
    ...
    if(taskdata->groupID!=0){
      //Restart groupID to 0, just in case this task structure is reused
  ...
  __kmp_thread_free(thread, taskdata);
    ...
    if (taskdata->tdg->tdgStatus == KMP_TDG_PREALLOC) {
    ...
    if(!(taskdata->tdg->tdgStatus== KMP_TDG_RECORDING)){
...
static void __kmp_task_finish(kmp_int32 gtid, kmp_task_t *task,
  ...
  ///Task count
  #if LIBOMP_TASKGRAPH
  __kmp_acquire_bootstrap_lock(&l_tasks);
  if(!(taskdata->td_flags).hidden_helper) --TASKS;//printf("--tasks\n");}
  __kmp_release_bootstrap_lock(&l_tasks);
  #endif
  /////////////////
...
kmp_task_t *__kmp_task_alloc(ident_t *loc_ref, kmp_int32 gtid,
  ...
  //Task counter
  #if LIBOMP_TASKGRAPH
  __kmp_acquire_bootstrap_lock(&l_target_tasks);
  __kmp_acquire_bootstrap_lock(&l_tasks);
  if(flags->hidden_helper) ++TARGET_TASKS;//printf("++targets\n");} 
  else ++TASKS;//printf("++tasks\n");}
  __kmp_release_bootstrap_lock(&l_target_tasks);
  __kmp_release_bootstrap_lock(&l_tasks);
  /////////////
  #endif
...
#define DYNAMIC_VARIANT_WARMUP 1
#if DYNAMIC_VARIANT_WARMUP
#include <sys/time.h>

#define STACK_SIZE 7192000
#define STACK_FILL_VALUE 0xaa

int warmup_num_iterations = 0;
int warmup_current_num_iterations = 0;

bool in_instrumentation = false;
bool in_variant = false;
// Definition of volatile_memset to avoid being optimized out while painting the
// stack
void *(*const volatile volatile_memset)(void *, int, size_t) = memset;

kmp_bootstrap_lock_t instrument_mem =
    KMP_BOOTSTRAP_LOCK_INITIALIZER(instrument_mem);

int current_num_openmp_threads = -1;
int current_pos_list_openmp_threads = 0;
int total_extra_exec = 0;
int num_threads_per_variant = 1;
int *list_of_openmp_threads = nullptr;

int warmup_num_variants = 0;
struct memory_reserved *mem_allocations;
struct timeval start, end;

size_t mem_allocations_size = 0;
int mem_allocations_num = 0;
long heap_bytes = 0;
long peak_heap_bytes = 0;
volatile double peak_cpu_usage = 0.0;
volatile double total_cpu_usage = 0.0;
volatile long peak_gpu_bytes = 0;
volatile long total_gpu_usage = 0;
unsigned long long garbage_gpu_bytes = 0;
volatile int sampling_count = 0;
uintptr_t stack_top = 0;

pthread_t resources_monitor;

#if NVML
bool nvml_initialized = false;
nvmlReturn_t result;
nvmlDevice_t device;
void __kmp_init_nvml() {
  result = nvmlInit();
...
void __kmp_deinit_nvml() { nvmlShutdown(); }

int __kmp_get_gpu_utilization() {
...
unsigned long long __kmp_get_gpu_mem() {
...
void __kmp_init_instrument() {
  ...
  sampling_count = 0;
  stack_top = 0;
  mem_allocations_size = 0;
  mem_allocations_num = 0;
  total_gpu_usage = 0;
  peak_gpu_bytes = 0;
  garbage_gpu_bytes = 0;
}

struct memory_reserved {
  void *addr;
  size_t size;
  int freed;
};
...
struct variant_data {
  ...
  double average_gpu_usage;
  long peak_gpu_bytes;
  long stack_usage;
  long peak_heap_bytes;
  double execution_time;
  int num_openmp_threads;
  int num_times;
};
struct variant_data variants[10];
void stack_initialize_fill_value(uintptr_t stack_top, size_t stack_size) {
  volatile_memset((void *)stack_top, STACK_FILL_VALUE, stack_size);
}

struct variant_data *__kmp_find_variant_data(char variant_name[], int num_openmp_threads) {
  for (int i = 0; i < warmup_num_variants; i++) {
    if (strcmp(variant_name, variants[i].variant_name) == 0 && variants[i].num_openmp_threads==num_openmp_threads) {
      return &variants[i];
    }
  }
  return nullptr;
...
void paint_stack() {
  ...
  stack_initialize_fill_value(stack_top, STACK_SIZE);
}

double __kmp_get_current_cpu_usage(long long int *prev_idle,  long long int *prev_total) {
  FILE *fp;
  char buffer[64];
  ...
  result = fgets(buffer, 64, fp);
  sscanf(buffer, "cpu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu", &user,
...
void __kmpc_malloc_cpu_mem(void *addr, size_t size) {
  if (in_variant) {
    __kmp_acquire_bootstrap_lock(&instrument_mem);
    //Check if the addr is already stored, to avoid counting duplications
    for (int i = 0; i < mem_allocations_num; i++) {
      if (mem_allocations[i].addr == addr && mem_allocations[i].freed == 0) {
         __kmp_release_bootstrap_lock(&instrument_mem);
        return;
      }
    }
    if (!mem_allocations_size) {
      mem_allocations =
          (struct memory_reserved *)real_malloc(sizeof(struct memory_reserved) * 10);
      mem_allocations_size = 10;
    } else if (mem_allocations_num == (int)mem_allocations_size) {
      mem_allocations = (struct memory_reserved *)realloc(
          mem_allocations, sizeof(struct memory_reserved) * (mem_allocations_size * 2));
      mem_allocations_size = mem_allocations_size * 2;
    }
    struct memory_reserved new_allocation = {addr, size, 0};
    mem_allocations[mem_allocations_num] = new_allocation;
    heap_bytes += size;
...
void __kmpc_free_cpu_mem(void *addr) {
  if (in_variant){
  __kmp_acquire_bootstrap_lock(&instrument_mem);
  for (int i = 0; i < mem_allocations_num; i++) {
    if (mem_allocations[i].addr == addr && mem_allocations[i].freed == 0) {
      heap_bytes -= mem_allocations[i].size;
      mem_allocations[i].freed = 1;
    }
  }
  __kmp_release_bootstrap_lock(&instrument_mem);
  }
...
void *malloc(size_t size) {
  ...
  __kmpc_malloc_cpu_mem(mem, size);
  return mem;
}
...
void free(void *addr) {
  ...
  __kmpc_free_cpu_mem(addr);
  real_free(addr);
}

void *cpu_usage_monitor(void *arg) {
  long long int prev_idle, prev_total;
  prev_idle = -1;
  prev_total = -1;
  double accumulated_cpu_usage = 0.0;
  while (1) {

#if NVML
  int current_gpu_usage = __kmp_get_gpu_utilization();
  total_gpu_usage += current_gpu_usage;

  long current_gpu_mem = (long)__kmp_get_gpu_mem();
  if (current_gpu_mem > peak_gpu_bytes)
    peak_gpu_bytes = current_gpu_mem;
#endif
  // Call the function to get current CPU usage
  double current_cpu_usage =
      __kmp_get_current_cpu_usage(&prev_idle, &prev_total);
  if (current_cpu_usage > peak_cpu_usage)
    peak_cpu_usage = current_cpu_usage;

  accumulated_cpu_usage += current_cpu_usage;
  total_cpu_usage = accumulated_cpu_usage;
  sampling_count++;

  usleep(5 * 1000); // Sleep for INTERVAL_MS milliseconds
  }
  return NULL;
}

void __kmp_print_variant_stats(char variant_name[], double average_cpu_usage,
                               double average_gpu_usage, long stack_usage,
                               double elapsed_time) {
  printf("Variant name: %s\n", variant_name);
  printf("Average CPU Usage: %.2f%% (over %d samples)\n", average_cpu_usage,
         (sampling_count - 1));
  printf("Peak CPU Usage: %.2f%%\n", peak_cpu_usage);
  printf("Average GPU Usage: %.2f%% (over %d samples)\n", average_gpu_usage,
         (sampling_count - 1));
  printf("Estimated Thread Stack Mem: %lu bytes\n", stack_usage);
  printf("Estimated Peak Heap Mem: %lu bytes\n", peak_heap_bytes);
  printf("Estimated Peak GPU Mem: %lu bytes\n", peak_gpu_bytes);
  if(current_num_openmp_threads!=-1)
    printf("Num OpenMP threads: %d \n", current_num_openmp_threads);
  printf("Execution Time: %.0f milliseconds\n", elapsed_time);
}

void __kmp_write_variant_stats_to_file() {
  FILE *file = fopen("variants.c", "w");
  if (file == NULL) {
    perror("Error opening file");
    return;
  }
  fprintf(file, "struct variant_data {\n");
  fprintf(file, "    char variant_name[256];\n");
  fprintf(file, "    double average_cpu_usage;\n");
  fprintf(file, "    double peak_cpu_usage;\n");
  fprintf(file, "    double average_gpu_usage;\n");
  fprintf(file, "    long peak_gpu_bytes;\n");
  fprintf(file, "    long stack_usage;\n");
  fprintf(file, "    long peak_heap_bytes;\n");
  fprintf(file, "    double execution_time;\n");
  fprintf(file, "    int num_openmp_threads;\n");
  fprintf(file, "    int num_times;\n");
  fprintf(file, "};\n");
  fprintf(file, "extern void __kmpc_register_variants_data(struct variant_data "
                "*variants_data, int warmup_num_variants);\n");
  fprintf(file, "\nint warmup_num_variants = %d;\n\n", warmup_num_variants);
  fprintf(file, "struct variant_data variants[%d] = {\n", warmup_num_variants);
  for (int i = 0; i < warmup_num_variants; i++) {
    fprintf(file, "    {\"%s\", %f, %f, %f, %ld, %ld, %ld, %f , %d, %d}",
            variants[i].variant_name, variants[i].average_cpu_usage,
            variants[i].peak_cpu_usage, variants[i].average_gpu_usage,
            variants[i].peak_gpu_bytes, variants[i].stack_usage,
            variants[i].peak_heap_bytes, variants[i].execution_time,
            variants[i].num_openmp_threads, variants[i].num_times);
    // Add comma for all elements except the last one
    if (i < warmup_num_variants - 1) {
      fprintf(file, ",");
    }
    fprintf(file, "\n");
  }
  fprintf(file, "};\n\n");
  fprintf(file, "void __kmp_register_variants_data(){\n");
  fprintf(file, " __kmpc_register_variants_data(variants,%d);\n", warmup_num_variants);
  fprintf(file, "}\n");

  fclose(file);
}

void __kmpc_register_variants_data(struct variant_data *provided_variants_data,
                                   int provided_warmup_num_variants) {

  memcpy(provided_variants_data, variants,
         provided_warmup_num_variants * sizeof(variant_data));
  warmup_num_variants = warmup_num_variants;
}

void __kmpc_instrument_variant_start() {
  __kmp_init_instrument();
  #if NVML
	__kmp_init_nvml();
  garbage_gpu_bytes = __kmp_get_gpu_mem();
  #endif
  in_variant = true;
  if (pthread_create(&resources_monitor, NULL, cpu_usage_monitor, NULL) != 0) {
...
void __kmpc_instrument_variant_end(char variant_name[]) {
  gettimeofday(&end, NULL);
  // Cancel the monitor thread
  if (pthread_cancel(resources_monitor) != 0) {
    perror("pthread_cancel");
    exit(1);
  }
   #if NVML
	__kmp_deinit_nvml();
  peak_gpu_bytes -= (long) garbage_gpu_bytes;
  #endif
  double elapsed_time = (end.tv_sec - start.tv_sec) * 1000.0 +
                        (end.tv_usec - start.tv_usec) / 1000.0;
  double average_cpu_usage = 0.0;
  double average_gpu_usage = 0.0;
  if (sampling_count >= 2)
    average_cpu_usage = total_cpu_usage / (sampling_count - 1);
  if (sampling_count >= 1)
    average_gpu_usage = total_gpu_usage / (sampling_count);

  uintptr_t high_watermark = stack_high_watermark(stack_top, STACK_SIZE);
  long stack_usage = STACK_SIZE - (high_watermark - stack_top);
  in_variant = false;
  //If some mallocs are found free them
  if(mem_allocations){
    free(mem_allocations);
    mem_allocations = nullptr;
  }
  variant_data *target = __kmp_find_variant_data(variant_name, current_num_openmp_threads);
  if (!target) {
    variant_data new_variant = {"",
                                average_cpu_usage,
                                peak_cpu_usage,
                                average_gpu_usage,
                                peak_gpu_bytes,
                                ...
                                current_num_openmp_threads,
                                1};
    variants[warmup_num_variants] = new_variant;
    strcpy(variants[warmup_num_variants].variant_name, variant_name);
    warmup_num_variants++;
  } else {
    // If the variant is already recorded, then update the values
    double new_average_cpu_usage =
        (target->average_cpu_usage * target->num_times + average_cpu_usage) /
        (target->num_times + 1);
    double new_execution_time =
        (target->execution_time * target->num_times + elapsed_time) /
        (target->num_times + 1);
    target->average_cpu_usage = new_average_cpu_usage;
    target->execution_time = new_execution_time;

    target->peak_cpu_usage = (peak_cpu_usage > target->peak_cpu_usage)
                                 ? peak_cpu_usage
                                 : target->peak_cpu_usage;

    target->stack_usage =
        (stack_usage > target->stack_usage) ? stack_usage : target->stack_usage;

    target->peak_heap_bytes = (peak_heap_bytes > target->peak_heap_bytes)
                                  ? peak_heap_bytes
                                  : target->peak_heap_bytes;
    target->num_times = target->num_times + 1;
  }

  __kmp_print_variant_stats(variant_name, average_cpu_usage, average_gpu_usage,
                            stack_usage, elapsed_time);
}

void __kmpc_init_instrumentation() {
  //Sleep to enhance accuracy of results for the next variants
  if(warmup_current_num_iterations!=0)
    sleep(2);
  warmup_current_num_iterations++;
  in_instrumentation = true;
}

void __kmpc_finish_instrumentation() {
  if (warmup_current_num_iterations == warmup_num_iterations){
    in_instrumentation = false;
    __kmp_write_variant_stats_to_file();
  }
}

int __kmpc_get_num_executions(){
  return warmup_num_iterations;
}
...
kmp_tdg_status __kmp_get_tdg_status(kmp_int32 gtid) {
  for (int i = 0; i < __kmp_tdgCreationInfo_size; i++) {
    if (__kmp_tdgCreationInfo[i].gtid == gtid) {
      return __kmp_tdgCreationInfo[i].tdg->tdgStatus;
    }
  }
  return KMP_TDG_NONE;
}
...
kmp_tdg_info *__kmp_get_tdg(kmp_int32 gtid) {
  for (int i = 0; i < __kmp_tdgCreationInfo_size; i++) {
    if (__kmp_tdgCreationInfo[i].gtid == gtid) {
      return __kmp_tdgCreationInfo[i].tdg;
    }
  }
  return nullptr;
}
...
kmp_int32 __kmp_increment_counter(kmp_tdg_info *tdg) {
  for (int i = 0; i < __kmp_tdgCreationInfo_size; i++) {
    if (__kmp_tdgCreationInfo[i].tdg == tdg) {
      int counter = KMP_ATOMIC_INC(&__kmp_tdgCreationInfo[i].currentTaskGenID);
      return counter;
    }
  }
  return -1;
...
void __kmp_decrement_counter(kmp_tdg_info *tdg) {
  for (int i = 0; i < __kmp_tdgCreationInfo_size; i++) {
    if (__kmp_tdgCreationInfo[i].tdg == tdg) {
      KMP_ATOMIC_DEC(&__kmp_tdgCreationInfo[i].currentTaskGenID);
    }
  }
}

// Mark tasks created inside a taskgraph, to differenciate
// then, this is used to decide if task should be freed and also when task ends
...
kmp_int32 __kmpc_set_task_static_id(kmp_int32 gtid, kmp_task_t *task) {
  ...
  for (int i = 0; i < __kmp_tdgCreationInfo_size; i++) {
    if (__kmp_tdgCreationInfo[i].gtid == gtid) {
      ...
      taskdata->tdg = __kmp_tdgCreationInfo[i].tdg;
      taskdata->td_task_id = __kmp_tdgCreationInfo[i].currentTaskGenID;
      KMP_ATOMIC_INC(&__kmp_tdgCreationInfo[i].currentTaskGenID);
      break;
    }
  }
...
void __kmpc_prepare_taskwait(kmp_task_t *task, void *data, kmp_int32 groupID,
  ...
  if (!NumReplicasListSize) {
    ...
    for (int i = 0; i < InitalNumOfLists; i++) {
      ReplicationList new_replica = {
          -1, 0, 0, nullptr, nullptr, FALSE, spatialConstraint};
      listsOfReplicas[i] = new_replica;
    }
  }
  // Increment the list size if it is not enough
    ...
    for (int i = oldSize; i < NumReplicasListSize; i++) {
      ReplicationList new_replica = {-1, 0, 0, nullptr, nullptr, FALSE};
      listsOfReplicas[i] = new_replica;
    }
  }
  ...
  if (!ListToUse->numNodesSize) {
    ...
    for (int i = 0; i < ListToUse->numNodesSize; i++) {
      struct ReplicationNode new_node = {nullptr, nullptr, FALSE,
                                         FALSE,   -1,      -1};
      ListToUse->nodes[i] = new_node;
    }
  }
  // Increment the list size if it is not enough
    ...
    for (int i = oldSize; i < ListToUse->numNodesSize; i++) {
      struct ReplicationNode new_node = {nullptr, nullptr, FALSE, FALSE, -1, -1};
      ListToUse->nodes[i] = new_node;
    }
  }
  // Save node and increment the number of nodes
  struct ReplicationNode new_node = {task, data, FALSE, FALSE, -1, -1};
  ListToUse->nodes[ListToUse->numNodes] = new_node;
...
int parseOpenMPVariantsNumThreads(int **outArray) {
    char* value = getenv("OPENMP_VARIANTS_NUM_THREADS");
    if (value == NULL) {
    ...
    // Duplicate the string for safe manipulation
    ...
    int count = 1; // At least one number is present
    for (char* p = valueDup; *p; p++) {
        if (*p == ',') count++;
    }
    ...
    // Second pass: parse and store the integers
    char* token = strtok(valueDup, ",");
    int i = 0;
    while (token != NULL) {
        array[i++] = atoi(token);
        token = strtok(NULL, ",");
    ...
    free(valueDup); // Cleanup the duplicated string
    *outArray = array; // Set the output array
    return count; // Return the number of elements parsed
}

kmp_int32 __kmpc_dynamic_variant(char *variants_names[], kmp_int32 *traits,
                                 int numVariants, kmp_int32 *user_conditions) {

    if (in_instrumentation) {
        if (warmup_current_num_iterations == 1) {
      // Read env var and calculate num_threads_per_variant
      num_threads_per_variant =
          parseOpenMPVariantsNumThreads(&list_of_openmp_threads);
      int numOpenMPVariants = 0;
      int numNonOpenMPVariants = 0;
      for (int i = 0; i < numVariants; i++) {
        if (traits[i] == PARALLEL_VARIANT)
          numOpenMPVariants++;
        else
          numNonOpenMPVariants++;
      }
      int numTotal =
          numNonOpenMPVariants + numOpenMPVariants * num_threads_per_variant;
      if (warmup_num_iterations < numTotal)
        warmup_num_iterations = numTotal;
        }

        int index = warmup_current_num_iterations - total_extra_exec - 1;
        if (traits[index] == PARALLEL_VARIANT) {
      // TODO: current_pos_list_openmp_threads must be per variant call, not
      // global
      if (list_of_openmp_threads != nullptr) {
        current_num_openmp_threads =
            list_of_openmp_threads[current_pos_list_openmp_threads];
        ompc_set_num_threads(current_num_openmp_threads);
      }
      if (current_pos_list_openmp_threads != num_threads_per_variant - 1) {
        total_extra_exec++;
        current_pos_list_openmp_threads++;
      } else {
        current_pos_list_openmp_threads = 0;
      }
        } else
      current_num_openmp_threads = -1;
        return index % numVariants;
    }
...
#if DYNAMIC_VARIANT_WARMUP
  /*
  double min_cpu_usage = 100.0;
  int selected_variant = -1;
  for (int i = 0; i < numVariants; i++) {
    struct variant_data *variant_found =
        __kmp_find_variant_data(variants_names[i]);
    if (variant_found && variant_found->average_cpu_usage < min_cpu_usage) {
      min_cpu_usage = variant_found->average_cpu_usage;
      selected_variant = i;
    }
  }
  */
...
#endif
  /*
    FILE* ptr = fopen("/sys/devices/gpu.0/load", "r");

    float f_load = 1.f / (1 << SI_LOAD_SHIFT);
    float load_cpu = sys_info.loads[0] * f_load * 100/get_nprocs();
    float load_gpu = 0;
    float threshold_load_cpu = 90;
   */
  /* 
  ...
  /*
  char *conf1 = std::getenv("VARIANT_DEVICE_USAGE");
  char *conf2 = std::getenv("VARIANT_DEVICE_THRESHOLD");
  if(conf2) threshold_load_cpu = atoi(conf2);

  //Reads the GPU load value
  if(ptr != NULL)  {
    char str[5];
    fgets(str, 5, ptr);
    load_gpu = atoi(str)/10.0;
    fclose(ptr);
  }
  
  bool exists_trait_GPU = false;
  for (int i = 0; i < numVariants; i++) if(traits[i] == GPU) exists_trait_GPU = true;
*/
  ///*
  //printf("%s %s %f\n", conf1, conf2, threshold_load_cpu);
  //printf("%f %f %f\n", load_cpu, load_gpu, threshold_load_cpu);
  //printf("Num tasks: %d, num targets: %d\n", TASKS, TARGET_TASKS);
  //*/
  ...
  for (int i = 0; i < numVariants; i++) {
   ...
   if (traits[i] == LOW_MEMORY) {
    ...
    } /*else if (traits[i] == GPU) {
      if( conf1 && (strcmp(conf1,"lesser") == 0 || strcmp(conf1,"LESSER") == 0) && load_gpu <= load_cpu) {
	      SelectedTrait = i;
	      break;
      }
    } else if (traits[i] == CPU) {
      if(conf1 && (strcmp(conf1,"lesser") == 0 || strcmp(conf1,"LESSER") == 0) && (load_gpu > load_cpu || !exists_trait_GPU)) {
	      if(load_cpu < threshold_load_cpu) SelectedTrait = i;
	      else SelectedTrait = 0; //base case
	      break;
      }
    }*/
  }
...
// OMPT task begin
    ...
    if (taskdata->td_target_data.async_handle != NULL) {
      ...
      __kmp_acquire_bootstrap_lock(&l_target_tasks);
      if(taskdata->td_target_data.async_handle == NULL) --TARGET_TASKS; //printf("--targets\n");} 
      __kmp_release_bootstrap_lock(&l_target_tasks);
      ///////////////
...
kmp_int32 __kmp_omp_task(kmp_int32 gtid, kmp_task_t *new_task,
  ...
  if (new_taskdata->is_taskgraph && task_tdg->tdgStatus == KMP_TDG_RECORDING) {
    ...
    if (new_taskdata->td_task_id >= task_tdg->mapSize) {
      __kmp_acquire_bootstrap_lock(&task_tdg->graph_lock);
      if (new_taskdata->td_task_id >= task_tdg->mapSize) {
        kmp_uint oldSize = task_tdg->mapSize;
    ...
    if (task_tdg->recordMap[new_taskdata->td_task_id].task == nullptr) {
  ...
  if (new_taskdata->is_taskgraph && __kmp_tdg_static_schedule) {
    int threadID = task_tdg->recordMap[new_taskdata->td_task_id].static_thread;
    if (threadID != -1) {
      if (!disable_stealing)
...
#if LIBOMP_TASKGRAPH
  int recording = (__kmp_get_tdg_status(gtid) == KMP_TDG_RECORDING);
  if (recording) {
...
#if LIBOMP_TASKGRAPH
  if (__kmp_get_tdg_status(gtid) == KMP_TDG_RECORDING) {
    kmp_tdg_info *ThisTdg = __kmp_get_tdg(gtid);
    ThisTdg->rec_taskred_data =
...
void *__kmpc_task_reduction_get_th_data(int gtid, void *tskgrp, void *data) {
  ...
  int recording = 0;
  if (__kmp_tdgCreationInfo)
    recording =
      (thread->th.th_current_task->tdg->tdgStatus == KMP_TDG_RECORDING);
...
void sync_tdg_tasks_for_task_team(kmp_int32 gtid, kmp_task_team_t *other_tt,
  ...
  for (kmp_int32 i = 0; i < nthreads; ++i) {
    for (int tdg_id = 0; tdg_id < __kmp_ntdgs; ++tdg_id) {
...
kmp_task_t *__kmp_task_dup_alloc(kmp_info_t *thread, kmp_task_t *task_src
  ...
  __kmp_acquire_bootstrap_lock(&l_target_tasks);
  __kmp_acquire_bootstrap_lock(&l_tasks);
  if((taskdata->td_flags).hidden_helper) ++TARGET_TASKS; //printf("++target\n");}
  else ++TASKS; //printf("++tasks\n"); }
  __kmp_release_bootstrap_lock(&l_target_tasks);
  __kmp_release_bootstrap_lock(&l_tasks);
  #endif
  ...
  if (!taskdata->is_taskgraph || from_taskloop_recur) // non-tdg tasks' id should be set to -1 by KMP_GEN_TASK_ID
    taskdata->td_task_id = KMP_GEN_TASK_ID();
  else if (taskdata->is_taskgraph && taskdata_src->tdg->tdgStatus == KMP_TDG_RECORDING) 
